...
class VM_No_GUI
  ...
  def exec_one
    ...
    when :SUB_F;
      b = unpack_float(pop); a = unpack_float(pop); push(pack_float(a - b));
    when :ADD_F;
      b = unpack_float(pop); a = unpack_float(pop); push(pack_float(a + b));
    when :MUL_F;
      b = unpack_float(pop); a = unpack_float(pop); push(pack_float(a * b));
    ...
    when :OR;
      b = pop; a = pop; push((!(a.zero?) || !(b.zero?)) ? 1 : 0)
    when :AND;
      b = pop; a = pop; push((!(a.zero?) && !(b.zero?)) ? 1 : 0)
    when :NOT;
      push(pop == 1 ? 0 : 1)
    ...
    when :PEEK_TOP;
      addr_abs = pop; value = @mem[addr_abs]; push(value)
    ...
    when :PUSH_F;
      a = read_code; push(a)
    when :ALLOC;
      num = read_code; @sp += num
    when :STO;
      abs_addr = pop; value = pop; @mem[abs_addr] = value;
    when :CALL;
      exec_call(read_code)
    when :RET;
      exec_ret(0)
    ...
    when :JUMP;
      target = read_code; goto_code(target)
    when :JNZ;
      target = read_code
      cond_val = pop
      if cond_val != 0;
    ...
    when :PRINT;
      exec_print
    when :PRINT_P;
      value = pop; print(value)
    when :PRINT_I;
      value = pop; print(value)
    when :PRINT_F;
      value = unpack_float(pop); print '%.5f' % value
    when :READ_I;
      exec_read_i
    when :PRINT_VRAM;
      color = pop; y = pop; x = pop; @vram_buffer[x][y] = color;
    when :FLUSH_VRAM;
      exec_flush_vram;
    when :CLEAR_VRAM;
      reset_vram(@vram);
    when :SLEEP;
      value = unpack_float(pop); sleep(value);
    when :RAND;
      idx = read_code; to = pop; from = pop; rand = @rand_generator.rand(from..to); @mem[@fp + idx] = rand;
    else
      ; raise 'bad instruction %02x' % [opcode]
    end
  end
